#ifndef TEEMO_ATTACKING_H
#define TEEMO_ATTACKING_H
#include "Debug.h"
/*
...
public:
	...
	int FindPoisonedDuration(std::vector<int>& timeSeries, int duration)
	{
		...
		for (int i = 1; i < N; ++i)
		{
			res += end > timeSeries[i] ? (timeSeries[i] + duration - end) : duration;
			end = std::max(end, timeSeries[i] + duration);
		}

		std::cout << "TeemoAttacking FindPoisonedDuration for \"" << Debug::ToStr1D<int>()(timeSeries) << "\", \"" << duration << "\": " << res << std::endl;
		return res;
	}
};
/*
TeemoAttacking FindPoisonedDuration for "1, 4", "2": 4
TeemoAttacking FindPoisonedDuration for "1, 2", "2": 3
*/
#endif
