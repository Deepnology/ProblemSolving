...
public:
	...
	bool ExistArithmeitcThree(const std::vector<int> & v)//O(n^2) time
	{
		int N = v.size();
		bool res = false;
		...
		std::cout << "LAPSubseq ExistArithmeticThree for \"" << Debug::ToStr1D<int>()(v) << "\": " << res << std::endl;
		return res;
	}
	...
	int Len_DP2D(const std::vector<int> & v)
	{
		...
		*/
		...
		std::cout << "LAPSubseq Len_DP2D for \"" << Debug::ToStr1D<int>()(v) << "\": " << llap << std::endl;
		return llap;
	}
};
/*
LAPSubseq ExistArithmeticThree for "1, 7, 10, 13, 14, 19": 1
[rY][cX]
Row#0	= 0, 4, 3, 2, 2, 2
Row#1	= 0, 0, 3, 3, 2, 2
Row#2	= 0, 0, 0, 2, 2, 2
...
LAPSubseq Len_DP2D for "1, 7, 10, 13, 14, 19": 4
LAPSubseq ExistArithmeticThree for "1, 7, 10, 15, 27, 29": 1
[rY][cX]
Row#0	= 0, 2, 2, 3, 2, 2
Row#1	= 0, 0, 2, 2, 2, 2
Row#2	= 0, 0, 0, 2, 2, 2
...
LAPSubseq Len_DP2D for "1, 7, 10, 15, 27, 29": 3
LAPSubseq ExistArithmeticThree for "2, 4, 6, 8, 10": 1
[rY][cX]
Row#0	= 0, 5, 3, 2, 2
Row#1	= 0, 0, 4, 2, 2
Row#2	= 0, 0, 0, 3, 2
...
LAPSubseq Len_DP2D for "2, 4, 6, 8, 10": 5
*/
#endif