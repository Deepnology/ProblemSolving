#ifndef MAX_VACATION_DAYS_H
#define MAX_VACATION_DAYS_H
#include "Debug.h"
/*
...
The flights are represented as a N*N matrix (not necessary symmetrical), called flights representing the airline status from the city i to the city j.
...
public:
	...
	//flights[city_i][city_j]==1: can fly from city_i to city_j
	//days[city_i][week_j]: vacation days in city_i during week_j

	int DFS(std::vector<std::vector<int>> & flights, std::vector<std::vector<int>>& days)
	{
		int res = INT_MIN;
		recur(flights, days, 0, 0, 0, res);
		return res;
	}
	void recur(std::vector<std::vector<int>> & flights, std::vector<std::vector<int>> & days, int curCity, int curWeek, int curVcDays, int & maxVcDays)
	{
		int weeks = days[0].size();
		if (curWeek == weeks)
		{
			maxVcDays = std::max(maxVcDays, curVcDays);
			return;
		}
		int cities = flights.size();
		for (int i = 0; i < cities; ++i)
			if (i == curCity || flights[curCity][i])
			{
				recur(flights, days, i, curWeek + 1, curVcDays + days[i][curWeek], maxVcDays);
			}
	}

	int DP2D(std::vector<std::vector<int>> & flights, std::vector<std::vector<int>>& days)
	{
		int cities = flights.size();
		int weeks = days[0].size();
		std::vector<std::vector<int>> dp(weeks, std::vector<int>(cities, 0));
		std::vector<std::vector<int>> reach(weeks, std::vector<int>(cities, 0));

		//init first week for all cities
		...
		{
			for (int j = 0; j < cities; ++j)//curCity j
			{
				for (int k = 0; k < cities; ++k)//prevCity k
				{
					if (reach[i - 1][k] && (j == k || flights[k][j]))//curCity j can be reached
					{
						reach[i][j] = 1;
						dp[i][j] = std::max(dp[i][j], dp[i - 1][k] + days[j][i]);
					}
				}
			}
		}
		int res = 0;
		for (auto & i : dp[weeks - 1])
			res = std::max(res, i);
		return res;
	}
};
